%% Written by Ethan Stancliffe 07/2017
clear all
close all
load iSIM.mat; %load model
changeCobraSolver('gurobi'); %change solver (change to whatever solver you are using)

%%
iSIM_rxn_decompositon = struct(); %create structure
...
iSIM_rxn_decompositon.met_id = met_id;
iSIM_rxn_decompositon.rxn_id = rxn_id;
iSIM_rxn_decompositon.coeff = coeff;
iSIM_rxn_decompositon.met_compartment = met_compartment;
iSIM_rxn_decompositon.met_compount = met_compound;
iSIM_rxn_decompositon.cpd_name = cpd_name;
iSIM_rxn_decompositon.met_name = met_name;
disp(iSIM_rxn_decompositon)

%Output into text file
file = fopen('iSIM_rxn_decompositon.txt','w');
fprintf(file,'%s ',string(fieldnames(iSIM_rxn_decompositon)));
...
file = fopen('iSIM_smatrix_names.txt','w');
...
file = fopen('iSIM_smatrix_id.txt','w');
smatrix_id = [string(model.mets),smat];
smatrix_id = [' ',string(model.rxns)';smatrix_id];
fprintf(file,'%s %s %s %s %s %s %s %s %s %s\n',smatrix_id');
%% Make rxn info struct
iSIM_rxn_info = struct();
%assign rxn_id,rxn_name,lower and upper bound fields
iSIM_rxn_info.rxn_id = model.rxns;
iSIM_rxn_info.rxn_name = model.rxnNames;
iSIM_rxn_info.lb = model.lb;
iSIM_rxn_info.ub = model.ub;
%construct reactions
for i = 1 :numReact
    sym = '-->';
...
iSIM_rxn_info.rxn_formula = reactForm'; %assign reaction formula fields
disp(iSIM_rxn_info)
file = fopen('iSIM_rxn_info.txt','w');
fprintf(file,'%s %s %s %s %s\n',[string(iSIM_rxn_info.rxn_id),string(iSIM_rxn_info.rxn_name),string(iSIM_rxn_info.lb),string(iSIM_rxn_info.ub),...
    string(iSIM_rxn_info.rxn_formula)]');
fclose(file);
...
saveas(fig,'iSIM_smatrix','pdf')%save as pdf
smat = fliplr(smat);
%%      Perform flux variabilitiy analysis with percentage of objective function %%%%%%%%

fva_pct_result = ef_tbl_fva(0,model,iSIM_rxn_info,0);%perform initial fva
for i = 1:20
   fva_pct_result = ef_tbl_fva(i*5,model,fva_pct_result,1); %perform for all percentages
end
disp(fva_pct_result)
file = fopen('iSIM_fva_result_percentage.txt','w');%open file
...
fig = figure;
j = 1;
for r = 1:2 %reactions to plot
    rx = strcat('R',num2str(r));
    subplot(1,2,j)
    map = [.2,.2,.2;0,0,1;1,0,0];%make color map
    colormap(map);%apply color map
    k = strcmp((fva_pct_result.rxn_id),rx); %extract indexes for matching reactions
...
pos = get(fig,'Position'); %https://www.mathworks.com/matlabcentral/answers/12987-how-to-save-a-matlab-graphic-in-a-right-size-pdf
set(fig,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]) 
saveas(fig,'iSIM_fva_percentage','pdf')

%%
% Peform FVA again but with ATP increments this time
fva_inc_result = ef_tbl_fva(0,model,iSIM_rxn_info,0);%perform initial fva
for i = 1:16
   fva_inc_result = ef_tbl_fva(100*i/16,model,fva_inc_result,1); %perform for all increments
end
disp(fva_inc_result)
file = fopen('iSIM_fva_result_increment.txt','w');%open file
fprintf(file,'rxn_id fva_lb fva_ub rxn_name lb ub rxn_formula fva_pct fva_req fva_on\n')%print headers
...
for r = 1:2 %reactions to plot
    ...
    map = [.2,.2,.2;0,0,1;1,0,0];%make color map
    colormap(map);%apply color map
    k = strcmp((fva_inc_result.rxn_id) ,rx); %extract indexes for matching reactions
...
pos = get(fig,'Position'); %https://www.mathworks.com/matlabcentral/answers/12987-how-to-save-a-matlab-graphic-in-a-right-size-pdf
set(fig,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]) 
saveas(fig,'iSIM_fva_increment','pdf')
%%
...
pos = get(fig,'Position'); %https://www.mathworks.com/matlabcentral/answers/12987-how-to-save-a-matlab-graphic-in-a-right-size-pdf
set(fig,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]) 
saveas(fig,'iSIM_fva_increment_all','pdf')
...
iSIM_gene_ko = struct(iSIM_rxn_info); %create new data struct
iSIM_gene_ko.gene_id = model.grRules; % add gene_ids
dele = zeros(length(model.grRules),1);
for i =1:length(model.grRules)
    k = find(strcmp(model.grRules,model.genes(i)));
    dele(k) = sol(length(sol(:,1)),i);
end
iSIM_gene_ko.gene_ko_atp =dele; %add result of simulation (Flux through objective function)
disp(iSIM_gene_ko)
file = fopen('iSIM_gene_ko_screen.txt','w');%output result
fprintf(file,'gene_id gene_ko_atp rxn_id rxn_name lb ub rxn_formula\n')
fprintf(file,'%s %s %s %s %s %s %s\n',[string(iSIM_gene_ko.gene_id),string(iSIM_gene_ko.gene_ko_atp),string(iSIM_gene_ko.rxn_id),string(iSIM_gene_ko.rxn_name),string(iSIM_gene_ko.lb),string(iSIM_gene_ko.ub),...
    string(iSIM_gene_ko.rxn_formula)]');
fclose(file);
%%
...
for i = 1:numRows
    for j = 1:i-1
        ...
        atp1(z) = iSIM_gene_ko.gene_ko_atp(k1);%get corresponding single gene deletion atp result
        atp2(z) = iSIM_gene_ko.gene_ko_atp(k2);
...
disp(gene_ko2_rxns)
file = fopen('iSIM_gene_2ko_rxns.txt','w');%output result
...
file = fopen('iSIM_gene_2ko_tbl.txt','w');%output result
